...
{
    SetPalette(GetVisualStudioDarkPalette());
...
const TextEditor::Palette& TextEditor::GetVisualStudioDarkPalette()
{
    const static Palette p = {{
        0xffd4d4d4,  // Default
        0xffd69c56,  // Keyword            #569CD6
        0xffa8ceb5,  // Number             #B5CEA8
        0xff7891ce,  // String             #CE9178
        0xff7891ce,  // Char literal       #CE9178
        0xffd4d4d4,  // Punctuation
        0xffc086c5,  // Preprocessor       #C586C0
        0xffd4d4d4,  // Identifier
        0xffb0c94e,  // Known identifier   #4EC9B0
        0xfffedc9c,  // Preproc identifier #9CDCFE
        0xff55996a,  // Comment            #6A9955
        0xff55996a,  // Multi-line comment #6A9955
        0xff1e1e1e,  // Background
        0xffffffff,  // Cursor
        0x80784f26,  // Selection          #264F78
        0x804747f4,  // Error marker       #F44747
        0x40000080,  // Breakpoint
        0xff858585,  // Line number
        0x402e2d2a,  // Current line fill  #2A2D2E
        0x402e2d2a,  // Current line fill inactive
        0x40333333,  // Current line edge
    }};
...
TextEditor::UndoRecord::UndoRecord(const std::string& aAdded, const TextEditor::Coordinates aAddedStart, const TextEditor::Coordinates aAddedEnd, const std::string& aRemoved,
                                   const TextEditor::Coordinates aRemovedStart, const TextEditor::Coordinates aRemovedEnd, TextEditor::EditorState& aBefore,
...
static bool TokenizeHtmlTag(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
    ...
    if (out_end < in_end && *out_end == '/')
        ++out_end;

    while (out_end < in_end && (isalnum(*out_end) || *out_end == '-' || *out_end == '_'))
        ++out_end;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HTML()
{
    ...
    if (!inited)
    {
        static const char* const htmlKeywords[] = {
            "html",  "head",  "body", "title", "meta", "link", "script", "style",  "div",    "span",   "p",        "a",     "img",    "ul",     "ol",     "li",      "table",
            "thead", "tbody", "tr",   "td",    "th",   "form", "input",  "button", "select", "option", "textarea", "label", "header", "footer", "main",   "section", "article",
            "nav",   "aside", "h1",   "h2",    "h3",   "h4",   "h5",     "h6",     "br",     "hr",     "strong",   "em",    "small",  "iframe", "canvas", "svg"};

        for (auto& k : htmlKeywords)
            langDef.mKeywords.insert(k);

        static const char* const identifiers[] = {"id",       "class",  "name",   "type",    "value",       "href",     "src",     "alt",      "rel",
                                                  "target",   "style",  "width",  "height",  "placeholder", "disabled", "checked", "selected", "readonly",
                                                  "required", "method", "action", "content", "charset",     "lang",     "data"};
        ...
        langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool
        {
            ...
            else if (TokenizeHtmlTag(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Keyword;
            else if (TokenizeCStyleString(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::String;
            else if (TokenizeCStyleIdentifier(in_begin, in_end, out_begin, out_end))
            {
                const std::string token(out_begin, out_end);

                if (langDef.mKeywords.find(token) != langDef.mKeywords.end())
                    paletteIndex = PaletteIndex::Keyword;
                else if (langDef.mIdentifiers.find(token) != langDef.mIdentifiers.end())
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::CSharp()
{
    ...
    if (!inited)
    {
        static const char* const csharpKeywords[] = {"abstract", "as", "base", "bool", "break", "byte", "case", "catch", "char", "checked", "class", "const", "continue", "decimal",
                                                     ...
                                                     // newer/contextual
                                                     "async", "await", "var", "dynamic", "yield", "partial", "record", "init", "required", "file", "global", "nameof", "not", "and",
                                                     "or", "when", "where", "select", "from", "group", "orderby", "join", "let", "into", "get", "set", "add", "remove", "value"};

        for (auto& k : csharpKeywords)
            langDef.mKeywords.insert(k);

        static const char* const identifiers[] = {"System",
                                                  "Console",
                                                  "Math",
                                                  "DateTime",
                                                  "DateTimeOffset",
        ...
        {
            Identifier id;
            id.mDeclaration = "Built-in type or common .NET symbol";
            langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
        }
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HLSL()
{
    ...
    if (!inited)
    {
        ...
        static const char* const identifiers[] = {"abort",
                                                  "abs",
                                                  "acos",
                                                  "all",
                                                  "AllMemoryBarrier",
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::GLSL()
{
    ...
    if (!inited)
    {
        ...
        static const char* const identifiers[] = {"abort",   "abs",     "acos",    "asin",    "atan",    "atexit",   "atof",    "atoi",    "atol",    "ceil",
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::AngelScript()
{
    ...
    if (!inited)
    {
        ...
        static const char* const identifiers[] = {"cos",         "sin",         "tab",         "acos",       "asin",     "atan",    "atan2",    "cosh",
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Lua()
{
    ...
    if (!inited)
    {
        ...
        static const char* const identifiers[] = {"assert",     "collectgarbage", "dofile",       "error",       "getmetatable", "ipairs",       "loadfile",     "load",
