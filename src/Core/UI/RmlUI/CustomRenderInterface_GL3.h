#ifndef CORE_UI_RMLUI_CUSTOM_RENDER_INTERFACE_GL3_H_
#define CORE_UI_RMLUI_CUSTOM_RENDER_INTERFACE_GL3_H_

#include <RmlUi/Core/RenderInterface.h>
#include <RmlUi/Core/Types.h>
#include <bitset>

// Forward declarations (same as original)
...
{
    /**
     * Custom OpenGL 3 renderer for RmlUi with multi-framebuffer support.
     * 
     * This is a modified version of RmlUi's RenderInterface_GL3 that respects
     * the currently bound framebuffer instead of always rendering to FBO 0.
     * 
     * Key Modifications:
     * - BeginFrame(): Saves the currently bound framebuffer
     * - EndFrame(): Renders to the saved framebuffer instead of hardcoded FBO 0
     * 
     * This allows UI to render correctly to editor viewports, post-processing
     * targets, and other custom framebuffers.
     */
    class CustomRenderInterface_GL3 : public Rml::RenderInterface 
{
    public : 
        CustomRenderInterface_GL3();
        ~CustomRenderInterface_GL3();

    // Returns true if the renderer was successfully constructed.
    explicit operator bool() const
    {
        return static_cast<bool>(program_data);
    }
    ...
    // DREAMENGINE MODIFICATION: Store target framebuffer from BeginFrame
    int m_target_framebuffer = 0;
