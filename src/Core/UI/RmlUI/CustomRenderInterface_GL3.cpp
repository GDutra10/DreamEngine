...
static const char* const program_uniform_names[(size_t)UniformId::Count] = {
    "_translate",  "_transform", "_tex", "_color", "_color_matrix",   "_texelOffset",       "_texCoordMin", "_texCoordMax", "_texMask",
    "_weights[0]", "_func",      "_p",   "_v",     "_stop_colors[0]", "_stop_positions[0]", "_num_stops",   "_value",       "_dimensions"};
...
static void CheckGLError(const char* operation_name)
{
    ...
    if (error_code != GL_NO_ERROR)
    {
        static const Rml::Pair<GLenum, const char*> error_names[] = {
            {GL_INVALID_ENUM, "GL_INVALID_ENUM"}, {GL_INVALID_VALUE, "GL_INVALID_VALUE"}, {GL_INVALID_OPERATION, "GL_INVALID_OPERATION"}, {GL_OUT_OF_MEMORY, "GL_OUT_OF_MEMORY"}};
...
CustomRenderInterface_GL3::CustomRenderInterface_GL3()
{
    ...
    if (Gfx::CreateShaders(*mut_program_data))
    {
        ...
        fullscreen_quad_geometry = CustomRenderInterface_GL3::CompileGeometry(mesh.vertices, mesh.indices);
    }
}
...
CustomRenderInterface_GL3::~CustomRenderInterface_GL3()
{
    ...
    {
        CustomRenderInterface_GL3::ReleaseGeometry(fullscreen_quad_geometry);
...
void CustomRenderInterface_GL3::SetViewport(int width, int height, int offset_x, int offset_y)
{
...
void CustomRenderInterface_GL3::BeginFrame()
{
    RMLUI_ASSERT(viewport_width >= 1 && viewport_height >= 1);

    // ============================================================================
    // DREAMENGINE MODIFICATION: Save target framebuffer
    // ============================================================================
    glGetIntegerv(GL_DRAW_FRAMEBUFFER_BINDING, &m_target_framebuffer);
...
void CustomRenderInterface_GL3::EndFrame()
{
    ...
    // ============================================================================
    // DREAMENGINE MODIFICATION: Use saved target framebuffer
    // ============================================================================
    // Original: glBindFramebuffer(GL_FRAMEBUFFER, 0);
    // Modified: Use the framebuffer that was bound during BeginFrame()
    glBindFramebuffer(GL_FRAMEBUFFER, m_target_framebuffer);

    // Adjust viewport based on target
    if (m_target_framebuffer != 0)
    {
        glViewport(0, 0, viewport_width, viewport_height);
    }
    else
    ...
    glClearColor(glstate_backup.color_clear_value[0], glstate_backup.color_clear_value[1], glstate_backup.color_clear_value[2], glstate_backup.color_clear_value[3]);
    glColorMask(glstate_backup.color_writemask[0], glstate_backup.color_writemask[1], glstate_backup.color_writemask[2], glstate_backup.color_writemask[3]);
    ...
    glStencilOpSeparate(GL_FRONT, glstate_backup.stencil_front.fail, glstate_backup.stencil_front.pass_depth_fail, glstate_backup.stencil_front.pass_depth_pass);
    ...
    glStencilOpSeparate(GL_BACK, glstate_backup.stencil_back.fail, glstate_backup.stencil_back.pass_depth_fail, glstate_backup.stencil_back.pass_depth_pass);

    Gfx::CheckGLError("EndFrame");
}

void CustomRenderInterface_GL3::Clear()
{
    glClearColor(0, 0, 0, 1);
    glClear(GL_COLOR_BUFFER_BIT);
}

Rml::CompiledGeometryHandle CustomRenderInterface_GL3::CompileGeometry(Rml::Span<const Rml::Vertex> vertices, Rml::Span<const int> indices)
{
    ...
    glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Position, 2, GL_FLOAT, GL_FALSE, sizeof(Rml::Vertex), (const GLvoid*)(offsetof(Rml::Vertex, position)));

    glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::Color0);
    glVertexAttribPointer((GLuint)Gfx::VertexAttribute::Color0, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(Rml::Vertex), (const GLvoid*)(offsetof(Rml::Vertex, colour)));

    glEnableVertexAttribArray((GLuint)Gfx::VertexAttribute::TexCoord0);
    glVertexAttribPointer((GLuint)Gfx::VertexAttribute::TexCoord0, 2, GL_FLOAT, GL_FALSE, sizeof(Rml::Vertex), (const GLvoid*)(offsetof(Rml::Vertex, tex_coord)));
...
void CustomRenderInterface_GL3::RenderGeometry(Rml::CompiledGeometryHandle handle, Rml::Vector2f translation, Rml::TextureHandle texture)
{
...
void CustomRenderInterface_GL3::ReleaseGeometry(Rml::CompiledGeometryHandle handle)
{
    Gfx::CompiledGeometryData* geometry = (Gfx::CompiledGeometryData*)handle;
...
void CustomRenderInterface_GL3::SetScissor(Rml::Rectanglei region, bool vertically_flip)
{
...
void CustomRenderInterface_GL3::EnableScissorRegion(bool enable)
{
...
void CustomRenderInterface_GL3::SetScissorRegion(Rml::Rectanglei region)
{
    SetScissor(region);
}

void CustomRenderInterface_GL3::EnableClipMask(bool enable)
{
    if (enable)
        glEnable(GL_STENCIL_TEST);
    else
...
void CustomRenderInterface_GL3::RenderToClipMask(Rml::ClipMaskOperation operation, Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation)
{
...
Rml::TextureHandle CustomRenderInterface_GL3::LoadTexture(Rml::Vector2i& texture_dimensions, const Rml::String& source)
{
...
Rml::TextureHandle CustomRenderInterface_GL3::GenerateTexture(Rml::Span<const Rml::byte> source_data, Rml::Vector2i source_dimensions)
{
...
void CustomRenderInterface_GL3::DrawFullscreenQuad()
{
    RenderGeometry(fullscreen_quad_geometry, {}, CustomRenderInterface_GL3::TexturePostprocess);
}

void CustomRenderInterface_GL3::DrawFullscreenQuad(Rml::Vector2f uv_offset, Rml::Vector2f uv_scaling)
{
    Rml::Mesh mesh;
    Rml::MeshUtilities::GenerateQuad(mesh, Rml::Vector2f(-1), Rml::Vector2f(2), {});
    ...
    RenderGeometry(geometry, {}, CustomRenderInterface_GL3::TexturePostprocess);
...
static void SetTexCoordLimits(GLint tex_coord_min_location, GLint tex_coord_max_location, Rml::Rectanglei rectangle_flipped, Rml::Vector2i framebuffer_size)
{
...
void CustomRenderInterface_GL3::RenderBlur(float sigma, const Gfx::FramebufferData& source_destination, const Gfx::FramebufferData& temp, const Rml::Rectanglei window_flipped)
{
    ...
    SetTexCoordLimits(GetUniformLocation(UniformId::TexCoordMin), GetUniformLocation(UniformId::TexCoordMax), scissor, {source_destination.width, source_destination.height});
...
void CustomRenderInterface_GL3::ReleaseTexture(Rml::TextureHandle texture_handle)
{
    glDeleteTextures(1, (GLuint*)&texture_handle);
}

void CustomRenderInterface_GL3::SetTransform(const Rml::Matrix4f* new_transform)
{
...
Rml::CompiledFilterHandle CustomRenderInterface_GL3::CompileFilter(const Rml::String& name, const Rml::Dictionary& parameters)
{
...
void CustomRenderInterface_GL3::ReleaseFilter(Rml::CompiledFilterHandle filter)
{
...
Rml::CompiledShaderHandle CustomRenderInterface_GL3::CompileShader(const Rml::String& name, const Rml::Dictionary& parameters)
{
...
void CustomRenderInterface_GL3::RenderShader(Rml::CompiledShaderHandle shader_handle, Rml::CompiledGeometryHandle geometry_handle, Rml::Vector2f translation,
...
void CustomRenderInterface_GL3::ReleaseShader(Rml::CompiledShaderHandle shader_handle)
{
    delete reinterpret_cast<CompiledShader*>(shader_handle);
}

void CustomRenderInterface_GL3::BlitLayerToPostprocessPrimary(Rml::LayerHandle layer_handle)
{
...
void CustomRenderInterface_GL3::RenderFilters(Rml::Span<const Rml::CompiledFilterHandle> filter_handles)
{
    for (const Rml::CompiledFilterHandle filter_handle : filter_handles)
    {
        ...
        switch (type)
        {
            ...
            case FilterType::DropShadow:
            {
                ...
                const Rml::Rectanglei window_flipped = VerticallyFlipped(scissor_state, viewport_height);
                SetTexCoordLimits(GetUniformLocation(UniformId::TexCoordMin), GetUniformLocation(UniformId::TexCoordMax), window_flipped, {primary.width, primary.height});
...
Rml::LayerHandle CustomRenderInterface_GL3::PushLayer()
{
...
void CustomRenderInterface_GL3::CompositeLayers(Rml::LayerHandle source_handle, Rml::LayerHandle destination_handle, Rml::BlendMode blend_mode,
...
void CustomRenderInterface_GL3::PopLayer()
{
    render_layers.PopLayer();
    glBindFramebuffer(GL_FRAMEBUFFER, render_layers.GetTopLayer().framebuffer);
}

Rml::TextureHandle CustomRenderInterface_GL3::SaveLayerAsTexture()
{
...
Rml::CompiledFilterHandle CustomRenderInterface_GL3::SaveLayerAsMaskImage()
{
    BlitLayerToPostprocessPrimary(render_layers.GetTopLayerHandle());
...
void CustomRenderInterface_GL3::UseProgram(ProgramId program_id)
{
...
int CustomRenderInterface_GL3::GetUniformLocation(UniformId uniform_id) const
{
    return program_data->uniforms.Get(active_program, uniform_id);
}

void CustomRenderInterface_GL3::SubmitTransformUniform(Rml::Vector2f translation)
{
...
CustomRenderInterface_GL3::RenderLayerStack::RenderLayerStack()
{
...
Rml::LayerHandle CustomRenderInterface_GL3::RenderLayerStack::PushLayer()
{
    ...
    if (layers_size == (int)fb_layers.size())
    {
        ...
        Gfx::CreateFramebuffer(fb_layers.back(), width, height, RMLUI_NUM_MSAA_SAMPLES, Gfx::FramebufferAttachment::DepthStencil, shared_depth_stencil);
    }
...
void CustomRenderInterface_GL3::RenderLayerStack::PopLayer()
{
    RMLUI_ASSERT(layers_size > 0);
    layers_size -= 1;
}

const Gfx::FramebufferData& CustomRenderInterface_GL3::RenderLayerStack::GetLayer(Rml::LayerHandle layer) const
{
    RMLUI_ASSERT((size_t)layer < (size_t)layers_size);
    return fb_layers[layer];
}

const Gfx::FramebufferData& CustomRenderInterface_GL3::RenderLayerStack::GetTopLayer() const
{
    return GetLayer(GetTopLayerHandle());
}

Rml::LayerHandle CustomRenderInterface_GL3::RenderLayerStack::GetTopLayerHandle() const
{
    RMLUI_ASSERT(layers_size > 0);
    return static_cast<Rml::LayerHandle>(layers_size - 1);
}

void CustomRenderInterface_GL3::RenderLayerStack::SwapPostprocessPrimarySecondary()
{
    std::swap(fb_postprocess[0], fb_postprocess[1]);
}

void CustomRenderInterface_GL3::RenderLayerStack::BeginFrame(int new_width, int new_height)
{
    RMLUI_ASSERT(layers_size == 0);
...
void CustomRenderInterface_GL3::RenderLayerStack::EndFrame()
{
    RMLUI_ASSERT(layers_size == 1);
    PopLayer();
}

void CustomRenderInterface_GL3::RenderLayerStack::DestroyFramebuffers()
{
...
const Gfx::FramebufferData& CustomRenderInterface_GL3::RenderLayerStack::EnsureFramebufferPostprocess(int index)
{
...
const Rml::Matrix4f& CustomRenderInterface_GL3::GetTransform() const
{
    return transform;
}

void CustomRenderInterface_GL3::ResetProgram()
{
    UseProgram(ProgramId::None);
}
