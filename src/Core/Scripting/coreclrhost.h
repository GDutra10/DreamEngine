...
//
CORECLR_HOSTING_API(coreclr_initialize, const char* exePath, const char* appDomainFriendlyName, int propertyCount, const char** propertyKeys, const char** propertyValues,
...
//  S_OK
//
CORECLR_HOSTING_API(coreclr_set_error_writer, coreclr_error_writer_callback_fn errorWriter);
...
//  HRESULT indicating status of the operation. S_OK if the assembly was successfully executed
//
CORECLR_HOSTING_API(coreclr_shutdown_2, void* hostHandle, unsigned int domainId, int* latchedExitCode);
...
//  HRESULT indicating status of the operation. S_OK if the assembly was successfully executed
//
CORECLR_HOSTING_API(coreclr_create_delegate, void* hostHandle, unsigned int domainId, const char* entryPointAssemblyName, const char* entryPointTypeName,
...
//  HRESULT indicating status of the operation. S_OK if the assembly was successfully executed
//
CORECLR_HOSTING_API(coreclr_execute_assembly, void* hostHandle, unsigned int domainId, int argc, const char** argv, const char* managedAssemblyPath, unsigned int* exitCode);
